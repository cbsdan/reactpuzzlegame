...
export const GameProvider = ({ children }) => {
  ...
  const pollForUpdates = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/events?lastVersion=${currentVersion}&lastPlayersCount=${currentPlayersCount}&roomId=${currentRoom._id}&isAdmin=${userRole === "admin"}`,
      );
  ...
  const addPlayer = async (name) => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/rooms/${currentRoom._id}/players`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const removePlayer = async (playerId) => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/rooms/${currentRoom._id}/players/${playerId}`,
        {
          method: "DELETE",
        },
      );
  ...
  const exitGame = async () => {
    ...
    try {
      if (currentPlayer) {
        await fetch(
          `${API_URL}/api/rooms/${currentRoom._id}/players/${currentPlayer._id}`,
          {
            method: "DELETE",
          },
        );
  ...
  const submitGuess = async (guess, guessCount = 1) => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/rooms/${currentRoom._id}/guess`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  // Submit a text answer (used by Stickman Mystery and future games)
  const submitAnswer = async (answer, metadata = {}) => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/rooms/${currentRoom._id}/answer`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
